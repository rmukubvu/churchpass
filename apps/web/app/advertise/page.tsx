...
export default function AdvertisePage() {
  ...
  const market = useMemo(
    () => getAdMarket(form.countryCode) ?? AD_MARKETS[0]!,
    [form.countryCode]
  );
  ...
  const selectedPrice = market.prices[form.duration];
  const selectedPriceLabel = formatAdPrice(selectedPrice, market);
  const selectedDurationMeta = AD_DURATION_LABELS[form.duration];
      ...
      <main className="max-w-2xl mx-auto px-5 pt-[96px] pb-20">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-extrabold text-white mb-3">
            Reach thousands of
            <br />
            church event attendees
          </h1>
          <p className="text-white/40 text-sm max-w-md mx-auto leading-relaxed">
            Your ad rotates in the featured slider on our homepage — seen by people actively
            looking for church events, conferences, and revivals.
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-10">
          {[
            ...
            <div
              key={s.label}
              className="rounded-2xl bg-[#111118] border border-white/5 p-4 text-center"
            >
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          {DURATIONS.map((d) => (
            ...
            >
              {d === "two_weeks" && (
                <span className="text-[10px] font-bold uppercase tracking-wider text-indigo-400 bg-indigo-400/10 px-2 py-0.5 rounded-full mb-2 inline-block">
                  Popular
                </span>
              )}
              <p className="text-lg font-extrabold text-white">
                {formatAdPrice(market.prices[d], market)}
              </p>
              <p className="text-sm font-semibold text-white/70 mt-0.5">
                {AD_DURATION_LABELS[d].label}
              </p>
              <p className="text-xs text-white/30 mt-1">{AD_DURATION_LABELS[d].desc}</p>
            </button>
          ))}
        </div>

        <p className="text-center text-xs text-white/25 mb-6 -mt-4">
          Prices in {market.currency.toUpperCase()} for advertisers in {market.label}
        </p>
          ...
          {step === "info" && (
            <>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-sm text-white/50 block mb-1.5">
                    Country of advertiser <span className="text-red-400">*</span>
                  </label>
                  <select
                    className={inputCls}
                  ...
                  <p className="text-xs text-white/25 mt-1.5">
                    Billing currency: {market.currency.toUpperCase()} ({market.symbol})
                  </p>
                </div>
                <div>
                  <label className="text-sm text-white/50 block mb-1.5">
                ...
                <div>
                  ...
                  <input
                    className={inputCls}
                    type="email"
                    placeholder="ads@yourcompany.com"
              ...
              <button
                type="button"
                onClick={() => {
                  if (!form.advertiserName.trim() || !form.advertiserEmail.trim()) {
                    setError("Please fill in all fields.");
                    return;
                  }
                  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.advertiserEmail)) {
          ...
          {step === "creative" && (
            <>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-sm text-white/50 block mb-1.5">
                    Ad banner image <span className="text-red-400">*</span>
                  </label>
                  <AdImageUpload
                ...
                <div>
                  ...
                  <input
                    className={inputCls}
                    placeholder="Book your hotel near the conference"
                    maxLength={60}
                    value={form.headline}
                ...
                <div>
                  <label className="text-sm text-white/50 block mb-1.5">
                    Subheadline <span className="text-white/20 text-xs">Optional</span>
                  </label>
                  <input
                    className={inputCls}
                    placeholder="Save 20% on rooms near the venue"
                    maxLength={100}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-sm text-white/50 block mb-1.5">
                      Click URL <span className="text-red-400">*</span>
                    </label>
                    <input
                      className={inputCls}
                      placeholder="https://booking.com/churchpass"
                      value={form.clickUrl}
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="button"
                  onClick={() => {
                    if (!form.imageUrl.trim() || !form.headline.trim() || !form.clickUrl.trim()) {
                      setError("Upload an image and fill in headline and click URL.");
                      return;
                    }
                    if (!form.imageUrl.startsWith("http")) {
                      setError("Please upload your ad image before continuing.");
                      return;
                    }
                    setError(null);
                    setStep("review");
            ...
            <>
              ...
              <div className="rounded-xl overflow-hidden border border-white/8">
                <div className="relative aspect-[3/1] bg-[#0a0a0f]">
                  ...
                  <div className="absolute bottom-0 left-0 p-4">
                    <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-amber-500/20 border border-amber-500/30 text-amber-400 mb-2 inline-block">
                      Ad
                    </span>
                    ...
                    {form.subheadline && (
                      <p className="text-xs text-white/60 mt-0.5">{form.subheadline}</p>
                    )}
                    <span className="inline-block mt-2 text-xs font-semibold px-3 py-1 rounded-full bg-indigo-600 text-white">
                      {form.ctaText}
                    </span>
                  </div>
                </div>
              ...
              <div className="rounded-xl bg-white/3 border border-white/5 p-4 space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-white font-medium">{selectedDurationMeta.label}</span>
                </div>
              ...
              <p className="text-xs text-white/30 text-center">
                All ads are reviewed within 24 hours before going live. You&apos;ll receive an
                email confirmation. Payment is processed securely by Stripe in{" "}
                {market.currency.toUpperCase()}.
              </p>
              ...
              <div className="flex gap-3">
                ...
                >
                  {loading ? "Redirecting…" : `Pay ${selectedPriceLabel} →`}
                </button>
              </div>
            </>
          )}
        ...
        <p className="text-center text-xs text-white/20 mt-6">
          Questions? Email{" "}
          <a
            href="mailto:ads@churchpass.events"
            className="text-indigo-400 hover:text-indigo-300 transition-colors"
          >
            ads@churchpass.events
          </a>
        </p>
      </main>
    </div>
