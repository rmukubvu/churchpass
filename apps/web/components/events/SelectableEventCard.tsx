...
export function SelectableEventCard({
  ...
  const isConference = sessionCount > 0;
  const dateLabel = formatShortDate(event.startsAt);
  const [month, day] = dateLabel.split(" ");

  return (
    <div className="relative group">
      {/* Clickable overlay — for conferences navigates to detail, for events toggles selection */}
      {isConference ? (
        <Link
          ...
          aria-label={`View sessions for ${event.title}`}
        />
      ) : (
        <button
          type="button"
      ...
      >
        {/* Top-right badge: session count for conferences, checkbox for single events */}
        {isConference ? (
          <div className="absolute top-3 right-3 z-20 flex items-center gap-1 bg-indigo-600/90 backdrop-blur-sm text-white text-[10px] font-bold px-2 py-1 rounded-full border border-indigo-500/50">
        ...
        <div className="relative aspect-[16/9] w-full bg-[#252525] overflow-hidden">
          ...
          {/* Category badge — hidden when checkbox or session badge visible */}
        ...
        <div className="flex flex-col gap-2 p-4">
          ...
          <div className="mt-1 pt-3 border-t border-white/5 flex items-center justify-between">
            {isConference ? (
              <span className="text-xs uppercase tracking-wider font-medium text-indigo-400/70">
