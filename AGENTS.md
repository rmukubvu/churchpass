...
- **Supabase Auth** — `@supabase/ssr` cookies; `lib/auth/server.ts` + `lib/auth/client.tsx` provide Clerk-shaped `auth()`, `currentUser()`, `useAuth()`, `SignedIn`; `middleware.ts` refreshes sessions and guards `/admin`; tRPC `ctx.clerkUserId` holds Supabase `user.id`
- **Authorization** — `isChurchAdmin(slug)`, `isSystemAdmin()`, tRPC guards in `lib/auth/guards.ts`; matrix in `docs/TRPC_AUTH_MATRIX.md`
...
## Documentation
- `docs/RSVP_FLOW.md` — RSVP → email QR → check-in trace
- `docs/TRPC_AUTH_MATRIX.md` — per-procedure authorization audit
...
2. Rename `clerk_*` DB columns / `ctx.clerkUserId` to `userId`
3. Middleware guard for `/{slug}/admin` (defense in depth; pages already check `isChurchAdmin`)
...
export const myRouter = router({
  doThing: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      // ctx.clerkUserId is guaranteed non-null (Supabase user id)
      // ctx.db is the Drizzle instance
      await requireChurchAdminForEventId(ctx.db, input.eventId); // when church-scoped
    }),
});
```

### Supabase user_metadata (admin roles)
```json
{ "firstName": "...", "lastName": "...", "role": "user", "adminOf": ["church-slug"] }
{ "role": "superadmin" }
```
Set on sign-up or via `supabase.auth.updateUser` after church registration (`api/church/register`).
